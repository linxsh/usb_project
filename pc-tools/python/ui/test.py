...
#[0x08, 0x00], # col_start_low
    ...
    [0xd1, 0x10],  #every N
    [0xd2, 0x00],  # 7 aec enable 5 clore y mode 4skin weight 3 weight drop mode
...
if __name__ == '__main__':
    ...
    showy = ShowY(dim=(2,2))
    cmd.send_regs(flatten(reg_conf))
    wait = 0 ##1
    while (1):
        data = cmd.recv_data(wait)
        if data:
            filename = './pics/'+str(count)+'.yuv'
            print filename
            ...
            for val in data:
                ...
                fd.write(a)
            fd.close()
            showy.show(count, filename, (300, 400))
            wait = 0
            count += 1
            if count >= 4:
                count = 0
