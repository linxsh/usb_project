...
## command option
CMD_READ_OPT  =  0x00000001
CMD_WRITE_OPT =  0x00000002
CMD_ACK_OPT   =  0x00000003
CMD_READ1_OPT =  0x00000004 ##立刻读取数据
## command format
CMD_DATA_FMT  =  0x00000001
CMD_CMD_FMT   =  0x00000002
## command length
CMD_LENGTH = 16
...
def str_to_list(value):
    vlist = list(value)
    for i in range(len(vlist)/4):
        tmp0 = ord(vlist[4*i+0])
        tmp1 = ord(vlist[4*i+1])
        vlist[4*i+0] = ord(vlist[4*i+3])
        vlist[4*i+1] = ord(vlist[4*i+2])
        vlist[4*i+2] = tmp1
        vlist[4*i+3] = tmp0
    return vlist

def str_to_list1(value):
    vlist = list(value)
    for i in range(len(vlist)/4):
        vlist[4*i+0] = ord(vlist[4*i+0])
        vlist[4*i+1] = ord(vlist[4*i+1])
        vlist[4*i+2] = ord(vlist[4*i+2])
        vlist[4*i+3] = ord(vlist[4*i+3])
    return vlist
...
class UartCmd(object):
    def __init__(self, port, buand=115200):
        self.uart = serial.Serial(port, buand, timeout=3)

    def send_regs(self, reg):
        imsg  = int_to_list(CMD_MAGIC_FLAG)
        imsg += int_to_list(CMD_WRITE_OPT)
        imsg += int_to_list(CMD_CMD_FMT)
        imsg += int_to_list(len(reg))
        self.uart.write(imsg)
        omsg = str_to_list(self.uart.read(CMD_LENGTH))
        if list_to_int(omsg[0:4]) == CMD_MAGIC_FLAG:
            imsg = list(reg);
            self.uart.write(imsg)
            omsg = str_to_list(self.uart.read(CMD_LENGTH))
            if list_to_int(omsg[0:4]) == CMD_MAGIC_FLAG:
    ...
    def recv_data(self, wait=1):
        imsg  = int_to_list(CMD_MAGIC_FLAG)
        if wait :
            imsg += int_to_list(CMD_READ_OPT)
        else :
            imsg += int_to_list(CMD_READ1_OPT)
        imsg += int_to_list(CMD_CMD_FMT)
        imsg += int_to_list(0)
        self.uart.write(imsg)
        omsg = str_to_list(self.uart.read(CMD_LENGTH))
        if list_to_int(omsg[0:4]) == CMD_MAGIC_FLAG:
            data_len = list_to_int(omsg[12:16])
            if data_len != 0:
                imsg  = int_to_list(CMD_MAGIC_FLAG)
                imsg += int_to_list(CMD_READ_OPT)
                imsg += int_to_list(CMD_DATA_FMT)
                imsg +=  omsg[12:16]
                self.uart.write(imsg)
...
if __name__ == '__main__':
    cmd = UartCmd("/dev/ttyUSB0", 1500000)
    reg = [
        [0xfe, 0x00],
        [0x01, 0xfa],
        [0x02, 0x70],
